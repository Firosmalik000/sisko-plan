        ...
        try {
            return DB::transaction(function () use ($store, $actor, $saleId, $accountId, $items, $date, $notes, $idempotencyKey, $requestHash, $ipAddress): SaleReturn {
                ...
                if ($date->lt(CarbonImmutable::parse((string) $sale->occurred_at))) {
                    throw ValidationException::withMessages(['occurred_at' => __('The return time cannot be earlier than the sale.')]);
                }
                ...
                if (count($saleItemIds) !== count(array_unique($saleItemIds))) {
                    throw ValidationException::withMessages(['items' => __('Return items cannot be duplicated.')]);
                }
                ...
                if ($saleItems->count() !== count($items)) {
                    throw ValidationException::withMessages(['items' => __('The sale item was not found in this store or document.')]);
                }
                $calculatedItems = [];
                $totalRefund = '0.0000';
                $totalCogs = '0.0000';
                foreach ($items as $index => $item) {
                    ...
                    if (! $saleItem instanceof SaleItem || Decimal::compare($item['quantity'], '0', Decimal::QUANTITY_SCALE) <= 0) {
                        throw ValidationException::withMessages(["items.{$index}.quantity" => __('The return quantity must be greater than zero.')]);
                    }
                    ...
                    if (Decimal::compare($newReturnedQuantity, $saleItem->quantity, Decimal::QUANTITY_SCALE) > 0) {
                        throw ValidationException::withMessages(["items.{$index}.quantity" => __('The return quantity exceeds the remaining returnable quantity.')]);
                    }
                    ...
                    if (Decimal::compare($baseQuantity, '0', Decimal::QUANTITY_SCALE) <= 0) {
                        throw ValidationException::withMessages(["items.{$index}.quantity" => __('The converted return quantity is invalid.')]);
                    }
                    $calculatedItems[] = [
