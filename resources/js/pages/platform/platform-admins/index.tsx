...
import { PlatformTableLeadCell, PlatformTableLeadHeader } from '@/components/platform-table-lead-cell';
...
export default function PlatformAdmins({
    ...
    return (
        <>
                            ...
                            {admins.map((admin, index) => (
                                <tr key={admin.id}>
                                    <PlatformTableLeadCell
                                        ...
                                        label={admin.name}
                                        actions={
                                            !can_manage || admin.id === platformAdmin.id
                                                ? []
                                                : [
                                                      {
                                                          label: admin.is_active ? 'Nonaktifkan admin' : 'Aktifkan admin',
                                                          icon: Power,
                                    ...
                                    <td className="px-4 py-3 text-xs text-slate-500">
                                        {admin.last_login_at
                                            ? new Date(admin.last_login_at).toLocaleString(localeTag(), {
                                                  dateStyle: 'medium',
                                                  timeStyle: 'short',
                                              })
                                            : '-'}
            ...
            {can_manage && <AddAdminDialog open={createOpen} setOpen={setCreateOpen} />}
        </>
    );
}

function AddAdminDialog({ open, setOpen }: { open: boolean; setOpen: (open: boolean) => void }) {
    return (
...
function PermissionDialog({ admin, groups }: { admin: AdminItem; groups: PermissionGroup[] }) {
    const [open, setOpen] = useState(false);
    const form = useForm({ permissions: admin.permissions });
    const toggle = (permission: string, checked: boolean) => {
        form.setData(
            'permissions',
            checked ? [...form.data.permissions, permission] : form.data.permissions.filter((item) => item !== permission),
        );
    };
    const close = () => {
        if (!form.processing) {
            ...
            <DialogContent className="flex max-h-[92svh] w-[calc(100%-1rem)] flex-col gap-0 overflow-hidden rounded-2xl border-slate-200 bg-white p-0 shadow-2xl sm:max-w-2xl">
                ...
                <form
                    onSubmit={(event) => {
                        ...
                        form.put(`/super-admin/platform-admins/${admin.id}/permissions`, {
                            preserveScroll: true,
                ...
                >
                    <div className="grid min-h-0 gap-3 overflow-y-auto p-4 sm:grid-cols-2 sm:p-5">
                        ...
                        <InputError message={form.errors.permissions ?? form.errors['permissions.0']} className="sm:col-span-2" />
                    </div>
                    <DialogFooter className="border-t border-slate-200 px-4 py-3 sm:px-5">
