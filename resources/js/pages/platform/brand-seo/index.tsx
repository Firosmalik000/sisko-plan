...
export default function BrandSeoIndex({ settings, can_manage }: { settings: Settings; can_manage: boolean }) {
    const form = useForm({
    ...
    const addSocialLink = () => {
        if (form.data.social_links.length < 25) {
            form.setData('social_links', [...form.data.social_links, { platform: 'Instagram', url: '' }]);
        }
    };
    ...
    const updateSocialLink = (index: number, field: keyof SocialLink, value: string) => {
        form.setData(
            'social_links',
            form.data.social_links.map((link, itemIndex) => (itemIndex === index ? { ...link, [field]: value } : link)),
        );
    };
                        ...
                        <div className="grid gap-4 p-4 sm:grid-cols-2 sm:p-6">
                            ...
                            <Field label="URL situs" error={errors.site_url}>
                                <Input
                                    ...
                                    onChange={(event) => form.setData('site_url', event.target.value)}
                            ...
                            action={
                                can_manage ? (
                                    <Button
                                        ...
                                        disabled={form.data.social_links.length >= 25}
                                        className="min-h-10"
                                    >
                                    ...
                                    {form.data.social_links.map((link, index) => (
                                        ...
                                        >
                                            <div>
                                                ...
                                                />
                                                <datalist id={`social-platforms-${index}`}>
                        ...
                        <div className="grid gap-4 p-4 sm:p-6">
                            ...
                            <Field label="URL gambar saat dibagikan" error={errors.social_image_url}>
                                <Input
                                    ...
                                    onChange={(event) => form.setData('social_image_url', event.target.value)}
                            ...
                            <label className="flex cursor-pointer items-start gap-3 rounded-xl bg-slate-50 p-4">
                                <input
                                    ...
                                    onChange={(event) => form.setData('robots_index', event.target.checked)}
                                    disabled={!can_manage}
