                    ...
                    <table className="w-full min-w-[980px] text-left text-sm">
                        ...
                        <tbody className="divide-y divide-[#3b211b]/8">
                            {payments.data.map((payment, index) => (
                                <tr key={payment.public_id} className="transition hover:bg-[#ee4d2d]/[.025]">
                                    <PlatformTableLeadCell
                                        index={paginatedRowNumber(payments.current_page, payments.per_page, index)}
                                        label={payment.receipt_number}
                                    />
                                    <td className="px-5 py-4">
                                        <p className="font-mono font-bold text-[#3b211b]">{payment.receipt_number}</p>
                                        <p className="mt-1 text-xs text-slate-500">{dateTime(payment.paid_at)}</p>
                                    </td>
                                    <td className="px-5 py-4">
                                        <p className="font-semibold">{payment.account?.name ?? payment.store.name}</p>
                                        {payment.account && <p className="mt-1 text-xs text-slate-500">{payment.account.email}</p>}
                                    </td>
                                    <td className="px-5 py-4">
                                    ...
                                    <td className="px-5 py-4 text-slate-600">{payment.created_by ?? '-'}</td>
                                    <td className="px-5 py-4 text-right text-base font-black text-[#3b211b]">{money(payment.amount)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
...
function SummaryCard({
    ...
    return (
        <article className={`platform-panel flex items-center gap-4 p-4 ${accent ? 'border-[#ee4d2d]/60 bg-[#fff7f4]' : ''}`}>
            <span
