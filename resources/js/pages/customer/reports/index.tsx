...
export default function ReportsPage({
    ...
    const submit = (event: FormEvent) => {
        ...
        router.get('/reports', { start_date: startDate, end_date: endDate }, { preserveState: true, replace: true });
    };
            ...
            <main className="min-h-full bg-[linear-gradient(180deg,#fffaf7_0%,#fff3ef_100%)] px-3 py-4 text-[var(--app-ink)] sm:px-5 sm:py-5 lg:px-8">
                <div className="mx-auto max-w-7xl space-y-4">
                    <header className="rounded-[1.35rem] border border-[var(--app-ink)]/8 bg-white px-4 py-4 shadow-sm sm:px-5">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                            <div className="min-w-0">
                                <h1 className="text-2xl font-black tracking-[-0.04em]">Laporan Usaha</h1>
                    ...
                    <section className="grid grid-cols-2 gap-2.5 lg:grid-cols-5">
                        <SummaryCard label="Penjualan Bersih" value={performance.net_revenue} />
                        <SummaryCard label="HPP Bersih" value={performance.net_cogs} tone="expense" />
                        <SummaryCard label="Laba Kotor" value={performance.gross_profit} />
                        <SummaryCard label="Biaya Toko" value={performance.expenses} tone="expense" />
                        <SummaryCard label="Estimasi Laba Usaha" value={performance.estimated_profit} featured />
                    </section>
...
function PositionCard({ icon: Icon, label, value }: { icon: typeof Boxes; label: string; value: string }) {
    return (
    ...
    return (
        <article className="overflow-hidden rounded-[1.35rem] border border-[var(--app-ink)]/8 bg-white p-4 shadow-sm sm:p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <p className="text-[10px] font-bold tracking-[0.16em] text-[var(--muted-foreground)] uppercase">Tren Kinerja</p>
                    <h2 className="mt-1 text-lg font-black tracking-[-0.03em] sm:text-xl">Penjualan dan estimasi laba</h2>
                    <p className="mt-0.5 text-[11px] font-semibold text-[var(--muted-foreground)]">{period}</p>
                </div>
            ...
            {hasActivity ? (
                ...
                <div className="mt-4 flex min-h-40 flex-col items-center justify-center rounded-xl bg-[#fff3ef] px-4 text-center">
                    ...
                    <p className="mt-2 text-sm font-bold text-[var(--muted-foreground)]">Belum ada transaksi pada periode ini</p>
                </div>
            )}
        </article>
    );
    ...
    return (
        <article className="overflow-hidden rounded-[1.35rem] border border-[var(--app-ink)]/8 bg-white shadow-sm">
            <div className="flex items-center justify-between gap-3 border-b border-[var(--app-ink)]/8 px-4 py-4 sm:px-5">
                <div>
                    <p className="text-[10px] font-bold tracking-[0.16em] text-[var(--muted-foreground)] uppercase">Performa Produk</p>
                    <h2 className="mt-1 text-lg font-black tracking-[-0.03em] sm:text-xl">Produk terlaris</h2>
                </div>
            ...
            {products.length > 0 ? (
                    ...
                    <div className="hidden overflow-x-auto md:block">
                        <table className="w-full min-w-[760px] text-left text-sm">
                            ...
                            <tbody className="divide-y divide-[var(--app-ink)]/6">
                                {products.map((item) => (
                                    <tr key={item.product_name} className="hover:bg-[#fffdfc]">
                ...
                <div className="flex min-h-36 flex-col items-center justify-center px-4 text-center">
                    ...
                    <p className="mt-2 text-sm font-bold text-[var(--muted-foreground)]">Belum ada produk terjual pada periode ini</p>
                </div>
            )}
        </article>
    );
